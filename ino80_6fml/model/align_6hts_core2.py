...
aln = alignment(env)
for code in ('6hts_chainG_resid512-816', '6fml_chainG_resid1549-1701'):
    mdl = model(env, file=code)
    aln.append_model(mdl, atom_files=code, align_codes=code)

for (weights, write_fit, whole) in (((1., 0.0, 0., 0., 1., 0.), False, True ),
                                    ((1., 0.5, 1., 1., 1., 0.), False, True ),
                                    ((1., 1.0, 1., 1., 1., 0.), True , False)):
...
aln.write(file='tmp.pap', alignment_format='PAP')
...
aln.salign(rms_cutoff=1.0,
           ...
           gap_gap_score=0, gap_residue_score=0, dendrogram_file='tmp.tree',
